...
hyperlist_file_var = tk.StringVar()
rom_extension_var = tk.StringVar(value=".zip")
output_dir_var = tk.StringVar()

# Interface utilisateur
tk.Label(root, text="Fichier HyperList :").grid(row=0, column=0, sticky="w")
tk.Entry(root, textvariable=hyperlist_file_var, width=50).grid(row=0, column=1)
tk.Button(root, text="Parcourir", command=select_hyperlist_file).grid(row=0, column=2)

tk.Label(root, text="Extension des ROM (ex. '.zip') :").grid(row=1, column=0, sticky="w")
tk.Entry(root, textvariable=rom_extension_var, width=50).grid(row=1, column=1)

tk.Label(root, text="Dossier de sortie :").grid(row=2, column=0, sticky="w")
tk.Entry(root, textvariable=output_dir_var, width=50).grid(row=2, column=1)
tk.Button(root, text="Parcourir", command=select_output_directory).grid(row=2, column=2)

tk.Button(root, text="Convertir", command=run_conversion).grid(row=3, column=1, pady=10)
